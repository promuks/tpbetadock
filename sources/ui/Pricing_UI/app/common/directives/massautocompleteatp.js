...
.directive('massAutocomplete', ["$timeout", "$window", "$document", "$q", "LookupDataService", function ($timeout, $window, $document, $q,LookupDataService) {
  'use strict';
  ...
  return {
    ...
    controller: ["$scope", function ($scope) {
      ...
      function _position_autocomplete() {
        ...
        container.style.top = current_element[0].clientHeight + 20 + 'px';
      ...
      function _suggest(term, target_element) {
        ...
        //start ATP code changes
        var lookupType = target_element.attr("lookup");
        var current_option_method = current_options.suggest;
        if (typeof current_option_method == 'undefined') {
          //provide default
          current_options.suggest = suggest_delimited
        }
        //end ATP changes
       /* var MAXLENTHLOOKUP = 3;
        if(lookupType === 'carriers'){
          MAXLENTHLOOKUP = 2;
        }
        var termValid = false;
        if (typeof(term) === 'string' && term.length > 0){
          ...
          if(term.indexOf(',') > -1){
            ...
            lastTerm = termArray[termArray.length -1];
          }
          if(lastTerm.length < MAXLENTHLOOKUP){
            termValid = true;
          }
        }
        if(termValid){*/
        if (typeof(term) === 'string' && term.length > 0) {
          $q.when(current_options.suggest(term,lookupType),
            function suggest_succeeded(suggestions) {
              ...
              // to return to his original expression after suggestions were made.
                ...
                $scope.results = [{ value: 'No Results found', label: 'No Results found'}];
              }
            },
      ...
      //START ATP CHANGES
      function suggest_delimited(term,lookupType){
        var suggestions=[];

        var data = LookupDataService.lookupDataFromCache(lookupType);
        suggestions = getSuggestions(term,data);
      ...
      function getSuggestions(term,data) {

          var ix = term.lastIndexOf(','),
            lhs = term.substring(0, ix + 1),
            rhs = term.substring(ix + 1),
            suggestions = suggestData(data,rhs);
      ...
      function suggestData(list, term) {
        var q = term.toLowerCase().trim();
        var results = [];
        var codeArr = [];
        var descArr = [];
        // Find first 10 elements that start with `term`.
        for (var i = 0; i < list.length && codeArr.length < 10; i++) {
          var element = list[i];
          if (element.code.toLowerCase().indexOf(q) === 0) {
            codeArr.push({label: element.code + "  -  " + element.desc, value: element.code});
          }
          if (element.desc.toLowerCase().startsWith(q)) {
            descArr.push({label: element.code + "  -  " + element.desc, value: element.code});
          }
        }
        results = codeArr.concat(descArr);
        return results;
      }
    //END ATP CHANGES
          ...
          switch (e.keyCode) {
            ...
            case KEYS.DOWN:
              if (e.keyCode == KEYS.TAB) {
                $scope.show_autocomplete = false;
                //console.debug(current_element[0].tabIndex);
                //var tab = current_element[0].tabIndex+1;
                //current_element[0].tabIndex = tab;
                //current_element[0].focus();
                //current_element[0].nextSiblingElement.focus();
                //break;
              }
